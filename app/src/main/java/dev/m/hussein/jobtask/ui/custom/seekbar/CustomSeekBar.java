package dev.m.hussein.jobtask.ui.custom.seekbar;
...
public class CustomSeekBar  <T extends Number> extends ImageView {
    /**
     * Default color of a {@link CustomSeekBar}, #FF33B5E5. This is also known as "Ice Cream Sandwich" blue.
     */
    ...
    private OnCustomSeekBarChangeListener<T> listener;
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.mipmap.ic_progress_marker;
        int thumbPressed = R.mipmap.ic_progress_marker;
        int thumbDisabled = R.mipmap.ic_progress_marker;
        int thumbShadowColor;
        int defaultShadowColor = Color.argb(75, 0, 0, 0);
        int defaultShadowYOffset = (int) Utils.dp2px(getResources(), 2);
        int defaultShadowXOffset = (int) Utils.dp2px(getResources(), 0);
        int defaultShadowBlur = (int) Utils.dp2px(getResources(), 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            internalPad = Utils.dp2px(getResources(), INITIAL_PADDING_IN_DP);
            barHeight = Utils.dp2px(getResources(), LINE_HEIGHT_IN_DP);
            ...
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.CustomSeekBar, 0, 0);
            try {
                setRangeValues(
                        extractNumericValueFromAttributes(a, R.styleable.CustomSeekBar_absoluteMinValue, DEFAULT_MINIMUM),
                        extractNumericValueFromAttributes(a, R.styleable.CustomSeekBar_absoluteMaxValue, DEFAULT_MAXIMUM),
                        extractNumericValueFromAttributes(a, R.styleable.CustomSeekBar_step, DEFAULT_STEP)
                );
                showTextAboveThumbs = a.getBoolean(R.styleable.CustomSeekBar_valuesAboveThumbs, true);
                textAboveThumbsColor = a.getColor(R.styleable.CustomSeekBar_textAboveThumbsColor, Color.WHITE);
                singleThumb = a.getBoolean(R.styleable.CustomSeekBar_singleThumb, false);
                showLabels = a.getBoolean(R.styleable.CustomSeekBar_showLabels, true);
                internalPad = a.getDimensionPixelSize(R.styleable.CustomSeekBar_internalPadding, INITIAL_PADDING_IN_DP);
                barHeight = a.getDimensionPixelSize(R.styleable.CustomSeekBar_barHeight, LINE_HEIGHT_IN_DP);
                activeColor = a.getColor(R.styleable.CustomSeekBar_activeColor, ACTIVE_COLOR);
                defaultColor = a.getColor(R.styleable.CustomSeekBar_defaultColor, Color.GRAY);
                alwaysActive = a.getBoolean(R.styleable.CustomSeekBar_alwaysActive, false);

                Drawable normalDrawable = a.getDrawable(R.styleable.CustomSeekBar_thumbNormal);
                if (normalDrawable != null) {
                    thumbImage = BitmapUtil.drawableToBitmap(normalDrawable);
                }
                Drawable disabledDrawable = a.getDrawable(R.styleable.CustomSeekBar_thumbDisabled);
                if (disabledDrawable != null) {
                    thumbDisabledImage = BitmapUtil.drawableToBitmap(disabledDrawable);
                }
                Drawable pressedDrawable = a.getDrawable(R.styleable.CustomSeekBar_thumbPressed);
                if (pressedDrawable != null) {
                    thumbPressedImage = BitmapUtil.drawableToBitmap(pressedDrawable);
                }
                thumbShadow = a.getBoolean(R.styleable.CustomSeekBar_thumbShadow, false);
                thumbShadowColor = a.getColor(R.styleable.CustomSeekBar_thumbShadowColor, defaultShadowColor);
                thumbShadowXOffset = a.getDimensionPixelSize(R.styleable.CustomSeekBar_thumbShadowXOffset, defaultShadowXOffset);
                thumbShadowYOffset = a.getDimensionPixelSize(R.styleable.CustomSeekBar_thumbShadowYOffset, defaultShadowYOffset);
                thumbShadowBlur = a.getDimensionPixelSize(R.styleable.CustomSeekBar_thumbShadowBlur, defaultShadowBlur);

                activateOnDefaultValues = a.getBoolean(R.styleable.CustomSeekBar_activateOnDefaultValues, false);
            } finally {
                a.recycle();
            }
        }
        ...
        textSize = (int) Utils.dp2px(getResources(), DEFAULT_TEXT_SIZE_IN_DP);
        distanceToTop = (int) Utils.dp2px(getResources(), DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        textOffset = !showTextAboveThumbs ? 0 : (int) (this.textSize + Utils.dp2px(getResources(),
        ...
        // make CustomSeekBar focusable. This solves focus handling issues in case EditText widgets are being used along with the CustomSeekBar within ScrollViews.
        setFocusable(true);
     ...
     */
    @SuppressWarnings("unused")
    public void setOnCustomSeekBarChangeListener(OnCustomSeekBarChangeListener<T> listener) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (pressedThumb != null) {
                    ...
                    if (notifyWhileDragging && listener != null) {
                        listener.onCustomSeekBarValuesChanged(this, getSelectedMinValue(), getSelectedMaxValue());
                    }
                }
                break;
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = (int) (thumbImage.getHeight()
                        + (!showTextAboveThumbs ? 0 : Utils.dp2px(getResources(), HEIGHT_IN_DP))
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            if (!singleThumb) {
                ...
                int spacing = (int) Utils.dp2px(getResources(), TEXT_LATERAL_PADDING_IN_DP);
    ...
    /**
     * Overridden to save instance state when device orientation changes. This method is called automatically if you assign an id to the CustomSeekBar widget using the {@link #setId(int)} method. Other members of this class than the normalized min and max values don't need to be saved.
     */
    @Override
    ...
    /**
     * Overridden to restore instance state when device orientation changes. This method is called automatically if you assign an id to the CustomSeekBar widget using the {@link #setId(int)} method.
     */
    @Override
     ...
     *
     * @param <T> The Number type the CustomSeekBar has been declared with.
     * @author Stephan Tittel (stephan.tittel@kom.tu-darmstadt.de)
     */
    public interface OnCustomSeekBarChangeListener<T extends Number> {

        void onCustomSeekBarValuesChanged(CustomSeekBar<T> bar, T minValue, T maxValue);
    }
